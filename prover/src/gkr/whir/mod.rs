...
// - Prover starts with oracle of evaluations P0 of the original poly P(X) at some smooth domain L0
// - also we assume that we have an original claim that P(Y) = Z, that can also we rewritten as sumcheck claim
...
// - Prover and verifier can engage in more than 1 sumcheck steps (here the tradeoff is less steps later, but more accesses to P0 oracle)
...
// - As in FRI, verifier has oracle access to f1(Q_i) by accessing 2^3 corresponding elements in P0 (at L0) and folding them.
...
// - Verifier explicitly performs the last sumcheck, that would involve evaluating eq(x, small roots of unity), and eq(x, high powers of ys)